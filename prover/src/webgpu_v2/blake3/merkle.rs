...
pub async fn generate_merkle_tree_webgpu_blake3_256(
    helper: &WebGpuHelper,
    leaves: &[Blake3Digest<32>],
) -> Option<Vec<Blake3Digest<32>>> {
    ...
    let rpo_hash_leaves_pipeline =
        helper.device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            ...
            entry_point: "blake3_256_hash_leaves",
        });
    ...
    let rpo_hash_level_pipeline =
        helper.device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            ...
            layout: None,
            module: &helper.module,
            entry_point: "blake3_256_hash_level",
        });
    ...
    if let Ok(Ok(())) = receiver.recv_async().await {
        ...
        let result =
            unsafe { core::slice::from_raw_parts(data.as_ptr() as *mut Blake3Digest<32>, leaves.len()) };
